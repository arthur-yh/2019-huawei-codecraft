...
class CAR(object):
    def __init__(self,id_,from_,to_,speed_,planTime_):
        ...
        # **** self defined paraments ****#
        self.routeclose = []
        self.routemap = []
        self.realtime = planTime_
    #
...
class ROAD(object):
    def __init__(self,id_, length_, speed_, channel_, from_, to_, isDuplex_):
        ...
        # **** dynamic parameters ****#
        # absolute bucket
        self.forwardBucket = [[None for j in range(self.channel_)] for i in range(self.length_)]
        self.backwardBucket = [[None for j in range(self.channel_)] for i in range(self.length_)] if self.isDuplex_ else None
...
class CROSS(object):
    ...
    def outOfCarport(self):
        ...
        for i in range(self.readyCars.__len__()):
            ...
            # **** 如果车的出口车超过了0.5 那么就调度初始时间+1 ****#
            # b = 0
            # for j in range(road.receiveBucket.__len__()):
            #     b += road.receiveBucket[j].count(None)
            # b = min(road.carCapcity, b)
            # bili = (road.carCapcity - b) / road.carCapcity
            # if bili >= 0.5:
            #     CARDICT[carId].realtime += 1
            #     # CROSSDICT[CARDICT[carId].__from__()].carportInitial(CARDICT[carId].realtime, carId)
            #     if CARDICT[carId].realtime not in self.carport.keys():
            #         self.carport[CARDICT[carId].realtime] = [carId]
            #     else:
            #         self.carport[CARDICT[carId].realtime].append(carId)
            #     duche[0] += 1
            #     continue
            # **** 如果车的出口车超过了80 那么就调度初始时间+1 ****#
...
class Astar(object):
    ...
    def Astar_run(self):
        ...
        count_num_car = 0
        for carname in CARNAMESPACE[0:]:
            count_num_car += 1
            CARDICT[carname].routeclose.append(CARDICT[carname].__from__())
            # fp.write('(%s, %s' % (carname, CARDICT[carname].__planTime__()))
            # print('(%s, %s' % (carname, CARDICT[carname].__planTime__()), sep='', end='')
            continue_tmp = True
            node = CARDICT[carname].__from__()
            road_temp = 1
            changeroad = False
            while continue_tmp:
                node_x_H, node_y_H, changeroad = Astar.findFvalue(self, carname, node, road_temp, changeroad)
                self.G = []
                ###### 找到最小值F所在的road 并且判断它是否是单向的 能否通行 ######
                Astar.checktoolong(self, node_x_H, node_y_H, carname)
                road_temp, changeroad = Astar.checkoneroad(self, carname, changeroad)
                continue1_tmp = True
                while continue1_tmp:
                    if ROADDICT[road_temp].isDuplex_ == 1:
                        if ROADDICT[road_temp].from_ == node:
                            # fp.write(', %s' % road_temp)
                            # print(', %s' % road_temp, sep='', end='')
                            node = ROADDICT[road_temp].to_
                            continue1_tmp = False
                        elif ROADDICT[road_temp].to_ == node:
                            # fp.write(', %s' % road_temp)
                            # print(', %s' % road_temp, sep='', end='')
                            node = ROADDICT[road_temp].from_
                            continue1_tmp = False
                    else:
                        if ROADDICT[road_temp].from_ == node:
                            # fp.write(', %s' % road_temp)
                            ...
                            del CARDICT[carname].routemap[-1]
                            self.F[self.F.index(min(self.F))] = max(self.F)
                            road_temp = self.open[self.F.index(min(self.F))]
                            CARDICT[carname].routemap.append(road_temp)
                CARDICT[carname].routeclose.append(node)
                ###### 如果车辆到达了最终的cross 推出循环 ######
                if node == CARDICT[carname].__to__():
                    # fp.write(')\n')
                    # print(')\n', sep='', end='')
                    continue_tmp = False
        return count_num_car
        # fp.close()


    def findFvalue(self, carname, node, road_temp, changeroad):
        self.open = CROSSDICT[node].roadIds[:] #找到与路口相连的四条路的信息
        ###### 已经走过的路标记为-1 第一次除外######
        if road_temp != 1:
            self.open[self.open.index(road_temp)] = -1
        Astar.findGvalue(self, carname)
        node_x_H, node_y_H = Astar.findHvalue(self, carname, node)
        for i in range(4):
            self.F[i] = self.G[i] + self.H[i]
        if changeroad:
            self.F[self.F.index(min(self.F))] += 10
            changeroad = False
        return node_x_H, node_y_H, changeroad

    def findGvalue(self, carname):
        ######  顺时针搜索 ######
        for i in range(4):
            if self.open[i] == -1:
                self.G.append(1)
            else:
                self.G.append(ROADDICT[self.open[i]].carCapcity * min(CARDICT[carname].__speed__(), ROADDICT[self.open[i]].__speed__()))
        max_G = max(self.G)
        for x in range(4):
            self.G[x] = max_G/self.G[x]

    def findHvalue(self, carname, node):
        # if node % 10 == 0:
        #     current_x_H = node // 10 - 1
        #     current_y_H = 10
        # else:
        #     current_x_H = node // 10
        #     current_y_H = node % 10
        # if CARDICT[carname].__to__() % 10 == 0:
        #     destination_x_H = CARDICT[carname].__to__() // 10 - 1
        #     destination_y_H = 10
        # else:
        #     destination_x_H = CARDICT[carname].__to__() // 10
        #     destination_y_H = CARDICT[carname].__to__() % 10
        node_x_H = CROSSDICT[CARDICT[carname].__to__()].x - CROSSDICT[node].x
        node_y_H = CROSSDICT[CARDICT[carname].__to__()].y - CROSSDICT[node].y
        node_H = abs(node_x_H) + abs(node_y_H)
        if (node_x_H > 0) & (node_y_H > 0):
            self.H[:] = [node_H + 1, node_H - 1, node_H - 1, node_H + 1]
        elif (node_x_H > 0) & (node_y_H < 0):
            self.H[:] = [node_H - 1, node_H - 1, node_H + 1, node_H + 1]
        elif (node_x_H < 0) & (node_y_H < 0):
            self.H[:] = [node_H - 1, node_H + 1, node_H + 1, node_H - 1]
        elif (node_x_H < 0) & (node_y_H > 0):
            ...
            self.H[:] = [node_H + 1, node_H + 1, node_H + 1, node_H - 1]
        return node_x_H, node_y_H

    def checktoolong(self, node_x_H, node_y_H, carname):
        if CARDICT[carname].routeclose.__len__() >= 1:
            if (node_x_H > 0) & (node_y_H > 0):
                self.F[0], self.F[1], self.F[2], self.F[3] = self.F[0] + 20, self.F[1] + max(np.random.randint(0, 2, 1)), self.F[2] + max(np.random.randint(0, 2, 1)), self.F[3] + 20
            elif (node_x_H > 0) & (node_y_H < 0):
                self.F[0], self.F[1], self.F[2], self.F[3] = self.F[0] + max(np.random.randint(0, 2, 1)), self.F[1] + max(np.random.randint(0, 2, 1)), self.F[2] + 20, self.F[3] + 20
            elif (node_x_H < 0) & (node_y_H < 0):
                self.F[0], self.F[1], self.F[2], self.F[3] = self.F[0] + max(np.random.randint(0, 2, 1)), self.F[1] + 20, self.F[2] + 20, self.F[3] + max(np.random.randint(0, 2, 1))
            elif (node_x_H < 0) & (node_y_H > 0):
                self.F[0], self.F[1], self.F[2], self.F[3] = self.F[0] + 20, self.F[1] + 20, self.F[2] + max(np.random.randint(0, 2, 1)), self.F[3] + max(np.random.randint(0, 2, 1))
            elif (node_x_H == 0) & (node_y_H > 0):
                self.F[0], self.F[1], self.F[3] = self.F[0] + 20, self.F[1] + 6 + max(np.random.randint(1, 4, 1)), self.F[3] + 6 + max(np.random.randint(1, 4, 1))
            elif (node_x_H == 0) & (node_y_H < 0):
                self.F[1], self.F[2], self.F[3] = self.F[1] + 6 + max(np.random.randint(1, 4, 1)), self.F[2] + 20, self.F[3] + 6 + max(np.random.randint(1, 4, 1))
            elif (node_x_H > 0) & (node_y_H == 0):
                self.F[0], self.F[2], self.F[3] = self.F[0] + 6 + max(np.random.randint(1, 4, 1)), self.F[2] + 6 + max(np.random.randint(1, 4, 1)), self.F[3] + 20
            elif (node_x_H < 0) & (node_y_H == 0):
                self.F[0], self.F[1], self.F[2] = self.F[0] + 6 + max(np.random.randint(1, 4, 1)), self.F[1] + 20, self.F[2] + 6 + max(np.random.randint(1, 4, 1))

    def checkoneroad(self, carname, changeroad):
        ######  如果这条路宽度只有1 并且速度小于等于4 那么把这条路的F变大 不选择他 概率可调  ######
        if (ROADDICT[self.open[self.F.index(min(self.F))]].__channel__() == 1) & (ROADDICT[self.open[self.F.index(min(self.F))]].__speed__() <= 4):
            if max(np.random.randint(0, 4, 1)) == 1:
                self.F[self.F.index(min(self.F))] = max(self.F)
        if self.open.count(-1) == 3:
            road_temp = max(self.open)
        else:
            road_temp = self.open[self.F.index(min(self.F))]
        CARDICT[carname].routemap.append(road_temp)
        if CARDICT[carname].routemap.count(road_temp) == 2:
            changeroad = True
        return road_temp, changeroad

class simulation(object):
    def __init__(self):
        self.dead = False
    def step(self):
        print("time:%d  %d  %d  %d" % (TIME[0], CARDISTRIBUTION[0], CARDISTRIBUTION[1], CARDISTRIBUTION[2]))
        for crossId in CROSSNAMESPACE:
            CROSSDICT[crossId].setDone(False)
        ...
        # print(CARDISTRIBUTION,sum(CARDISTRIBUTION))
    def simulate(self):
...
#     crossInfo = open(cross_path, 'r').read().split('\n')[1:]
    ...
    for line in roadInfo:
        ...
        ROADDICT[int(id_)] = ROAD(int(id_), int(length_), int(speed_), int(channel_), int(from_), int(to_), int(isDuplex_))
...
# process
    ...
    for carname in CARNAMESPACE[:]:
        if (CARDICT[carname].routemap.__len__() > 18) & (CARDICT[carname].speed_ < 10):
            # type1 += 1
            CARDICT[carname].realtime += max(np.random.randint(0, 50, 1))
        elif (CARDICT[carname].routemap.__len__() <= 18) & (CARDICT[carname].routemap.__len__() > 15 ) & (CARDICT[carname].speed_ < 10):
            # type2 += 1
            CARDICT[carname].realtime += max(np.random.randint(150, 500, 1))
        elif (CARDICT[carname].routemap.__len__() <= 15) & (CARDICT[carname].routemap.__len__() > 13 ) & (CARDICT[carname].speed_ < 10):
            # type3 += 1
            CARDICT[carname].realtime += max(np.random.randint(500, 950, 1))
        elif (CARDICT[carname].routemap.__len__() >= 15) & (CARDICT[carname].speed_ >= 10):
            # type4 += 1
            CARDICT[carname].realtime += max(np.random.randint(1000, 2000, 1))
        elif (CARDICT[carname].routemap.__len__() <= 13) & (CARDICT[carname].speed_ <= 10):
            # type5 += 1
            CARDICT[carname].realtime += max(np.random.randint(2000, 2500, 1))
        elif (CARDICT[carname].routemap.__len__() > 13) & (CARDICT[carname].routemap.__len__() <= 15) & (CARDICT[carname].speed_ >= 10):
            # type6 += 1
            CARDICT[carname].realtime += max(np.random.randint(2500, 2800, 1))
        elif (CARDICT[carname].routemap.__len__() <= 13) & (CARDICT[carname].speed_ >= 10):
            # type7 += 1
            CARDICT[carname].realtime += max(np.random.randint(2850, 3000, 1))
    # print(type1, type2, type3, type4, type5, type6, type7, type1+type2+type3+type4+type5+type6+type7)
        # type1, type2, type3, type4, type5, type6, type7 = 0, 0, 0, 0, 0, 0, 0
    ...
    for carname in CARNAMESPACE[:]:
        ...
        carId = carname
        planTime_ = CARDICT[carname].realtime
        route = CARDICT[carname].routemap[:]
